...
const formatDatesForQuery = (startDate, endDate) => {
  ...
  // END: Final del día a las 23:59:59 (para incluir todo el último día)
  const end = moment(endDate).format("YYYY-MM-DD 23:59:59");

  // Para columnas tipo DATE (gastos)
...
async function getSummary(req, res) {
  try {
    ...
    // Obtener rangos del período anterior
    const {
      start: prevStart,
      end: prevEnd,
      dateOnlyStart: prevDateOnlyStart,
      dateOnlyEnd: prevDateOnlyEnd,
    } = formatDatesForQuery(previousStartDate, previousEndDate);

    // --- Consultas SQL Base (Periodo Actual y Anterior) ---
...
WHERE p.fecha_pago BETWEEN ? AND ? AND p.estado = 'confirmado'`.trim();

    const sqlExpensesBase = `SELECT
COALESCE(SUM(g.monto), 0) AS totalExpenses
FROM gastos g
WHERE g.fecha_gasto BETWEEN ? AND ?`.trim();

    const sqlEventsBase = `SELECT
COUNT(DISTINCT id_evento) AS totalEvents
FROM eventos
WHERE fecha_inicio BETWEEN ? AND ? AND estado = 'publicado'`.trim();

    // 2. Ejecutar consultas para el PERIODO ACTUAL
    const revenueCurrentP = queryDB(sqlRevenueBase, [start, end]);
    const expensesCurrentP = queryDB(sqlExpensesBase, [
      dateOnlyStart,
      dateOnlyEnd,
    ]);
    const eventsCurrentP = queryDB(sqlEventsBase, [start, end]);

    // 3. Ejecutar consultas para el PERIODO ANTERIOR
    const revenuePreviousP = queryDB(sqlRevenueBase, [prevStart, prevEnd]);
    const expensesPreviousP = queryDB(sqlExpensesBase, [
      prevDateOnlyStart,
      prevDateOnlyEnd,
    ]);
    const eventsPreviousP = queryDB(sqlEventsBase, [prevStart, prevEnd]);
    ...
    const totalEvents = parseInt(currentEv.totalEvents);
...
// --------------------------------------------------------------------------
// RUTA 2: /api/reports/sales (Reporte de Ventas Detallado)
// --------------------------------------------------------------------------
...
FROM pagos p
JOIN tickets t ON p.id_pago = t.id_pago
...
GROUP BY p.id_pago, p.fecha_pago, e.nombre, p.monto, m.nombre
ORDER BY p.fecha_pago DESC;`.trim();
    const sales = await queryDB(sql, [start, end]); // CLAVE: Pasar [start, end]
...
// --------------------------------------------------------------------------
// RUTA 3: /api/reports/expenses (Reporte de Gastos Detallado)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// RUTA 4: /api/reports/events (Reporte de Eventos)
// --------------------------------------------------------------------------
...
COALESCE(SUM(CASE WHEN p.estado = 'confirmado' THEN p.monto ELSE 0 END), 0) AS ingresos,
(COALESCE(SUM(CASE WHEN p.estado = 'confirmado' THEN 1 ELSE 0 END), 0) * 100.0 / e.capacidad_total) AS ocupacion
FROM eventos e
...
ORDER BY e.fecha_inicio DESC;`.trim();
    const events = await queryDB(sql, [start, end]); // CLAVE: Pasar [start, end]
...
// --------------------------------------------------------------------------
// RUTA 5: /api/reports/trend (Tendencia Diaria)
// --------------------------------------------------------------------------
async function getTrendData(req, res) {
  try {
    ...
    // Pagos (TIMESTAMP)
    const revenueSql =
      `SELECT DATE(fecha_pago) AS date, SUM(monto) AS value FROM pagos WHERE fecha_pago BETWEEN ? AND ? AND estado = 'confirmado' GROUP BY date ORDER BY date;`.trim();
    ...
    const expenses = await queryDB(expenseSql, [dateOnlyStart, dateOnlyEnd]); // CLAVE: Pasar [dateOnlyStart, dateOnlyEnd]

    // Lógica de Consolidación de Fechas
...
// --------------------------------------------------------------------------
// RUTA 6: /api/reports/payment-distribution (Distribución por Método de Pago)
// --------------------------------------------------------------------------
...
ORDER BY total DESC;`.trim();
    const distribution = await queryDB(sql, [start, end]); // CLAVE: Pasar [start, end]
...
// --------------------------------------------------------------------------
// RUTA 7: /api/reports/artists-performance (Rendimiento de Artistas)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// RUTA 8: /api/reports/payroll (Nómina de Artistas)
// --------------------------------------------------------------------------
async function getPayroll(req, res) {
  try {
    const { startDate, endDate } = req.query;
    const { dateOnlyStart, dateOnlyEnd } = formatDatesForQuery(startDate, endDate);
...
// --------------------------------------------------------------------------
// RUTA 9: /api/reports/reconciliation (Conciliación Bancaria)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// RUTA 10: /api/reports/financial-summary (Resumen Financiero - P&L)
// --------------------------------------------------------------------------
async function getFinancialSummary(req, res) {
  try {
    const { startDate, endDate } = req.query;

    const { start, end, dateOnlyStart, dateOnlyEnd } = formatDatesForQuery(startDate, endDate);

    // Calcular período anterior
    const startM = moment(startDate);
    const endM = moment(endDate);
    const duration = endM.diff(startM, "days") + 1;
    const previousEndDate = startM.clone().subtract(1, "day").format("YYYY-MM-DD");
    const previousStartDate = startM.clone().subtract(duration, "days").format("YYYY-MM-DD");

    const { start: prevStart, end: prevEnd, dateOnlyStart: prevDateOnlyStart, dateOnlyEnd: prevDateOnlyEnd } = formatDatesForQuery(previousStartDate, previousEndDate);
    ...
    const [expensesResult] = await queryDB(expensesSql, [dateOnlyStart, dateOnlyEnd]);
    ...
    const [prevExpensesResult] = await queryDB(expensesSql, [prevDateOnlyStart, prevDateOnlyEnd]);
