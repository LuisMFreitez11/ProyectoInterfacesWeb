...
// Obtener tipos de entrada (zonas) de un evento (usado por frontend AdminEvents)
router.get('/:id/tiposEntrada', async (req, res) => {
    try {
        const pool = getDB();
        const { id } = req.params;
        ...
        console.error('Error al obtener tipos de entrada del evento:', err);
        res.status(500).json({
            success: false,
            message: 'Error al obtener tipos de entrada del evento',
            error: err.message
        });
    }
});
